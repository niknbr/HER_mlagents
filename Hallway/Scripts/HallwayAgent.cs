...
public class HallwayAgent : Agent
{
    ...
    public override void CollectObservations()
    {
        if (useVectorObs)
        {
			...
			if(selection==0){
				detectableObjects = new string[] { "orangeGoal"};
				targetPosition = orangeGoal.transform.position;
			} else {
				detectableObjects = new string[] { "redGoal"};
				targetPosition = redGoal.transform.position;
			}
			// AddVectorObs(rayPer.Perceive(rayDistance, rayAngles, detectableObjects, 0f, 0f));
			AddVectorObs(targetPosition);

            AddVectorObs(agentRB.transform.position);
        }
    }
